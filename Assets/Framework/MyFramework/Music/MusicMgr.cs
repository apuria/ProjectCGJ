...
public class MusicMgr : SingletonAutoMono<MusicMgr>
{
    ...
    private void FixedUpdate()
    {
        ...
        // 不停的遍历容器 检测有没有音效播放完毕 播放完了 就移除归还它
        ...
        for (int i = soundList.Count - 1; i >= 0; --i)
        {
            if (!soundList[i].isPlaying)
            {
                ...
                _proxy.Push(soundList[i].gameObject);
    ...
    public void PlayBKMusic(string name)
    {
        ...
        // 使用 ResourcesLoader 异步加载背景音乐
        ResourcesLoader.Instance.LoadAsync<AudioClip>("music/" + name, (clip) =>
        {
            bkMusic.clip = clip;
            bkMusic.loop = true;
    ...
    public void PlaySound(string name, bool isLoop = false, bool isSync = false, UnityAction<AudioSource> callBack = null)
    {
        ...
        {
            // 同步加载音效资源
            AudioClip clip = ResourcesLoader.Instance.Load<AudioClip>("sound/" + name);
            PlaySoundWithClip(clip, isLoop, callBack);
        }
        else
        {
            // 异步加载音效资源
            ResourcesLoader.Instance.LoadAsync<AudioClip>("sound/" + name, (clip) =>
            {
                PlaySoundWithClip(clip, isLoop, callBack);
            });
        }
    ...
    /// <summary>
    /// 使用已加载的 AudioClip 播放音效（同步和异步加载后统一入口）
    /// </summary>
    private void PlaySoundWithClip(AudioClip clip, bool isLoop, UnityAction<AudioSource> callBack)
    {
        // 从缓存池中取出音效对象得到对应组件
        AudioSource source = _proxy.Get("Sound/soundObj").GetComponent<AudioSource>();
