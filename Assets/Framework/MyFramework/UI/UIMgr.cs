...
public class UIMgr : BaseManager<UIMgr>
{
    ...
    public void ShowPanel<T>(E_UILayer layer = E_UILayer.Middle, UnityAction<T> callBack = null, bool isSync = false) where T:BasePanel
    {
        ...
        //使用 ResourcesLoader 加载面板预设体
        string resPath = "UI/" + panelName;
        if (isSync)
        {
            GameObject res = ResourcesLoader.Instance.Load<GameObject>(resPath);
            OnPanelLoaded<T>(panelName, layer, res);
        }
        else
        {
            ResourcesLoader.Instance.LoadAsync<GameObject>(resPath, (res) =>
            {
                OnPanelLoaded<T>(panelName, layer, res);
            });
        }
    ...
    /// 面板资源加载完成后的通用处理逻辑
    /// </summary>
    private void OnPanelLoaded<T>(string panelName, E_UILayer layer, GameObject res) where T : BasePanel
    {
        //字典中不存在对应记录 说明面板已被移除
        ...
        if (panelInfo.isHide)
        {
            ...
            panelDic.Remove(panelName);
            //卸载不再使用的资源
            ResourcesLoader.Instance.UnloadAsset<GameObject>("UI/" + panelName, isDel: true);
            return;
        }
        ...
        if (res == null)
        {
            Debug.LogError($"面板资源加载失败: UI/{panelName}");
            panelDic.Remove(panelName);
            return;
        }
        ...
        if (panel == null)
        {
            Debug.LogError($"面板预设体上未找到组件: {typeof(T).Name}，请确认预设体名与类名一致");
